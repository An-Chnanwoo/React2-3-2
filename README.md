...
#### 9.3 Cypress를 사용한 엔드 투 엔드 테스트
  - 앱 전체가 잘 동작하는지 확인하는 테스트
  - 웹 브라우저에서 돌아가는 모든 것을 테스트할 수 있는 강력한 도구
  - 파이어폭스, 구글 크롬과 같은 크로미엄 기반 브라우저에서 실행
...
#### 9.1 테스트란
  - 단위테스트 : 각 함수가 작동하는지 확인하는 테스트
  - 엔드 투 엔드 테스트 : 특정 작동이 발행했을 때 적절한 응답을 확인하는 테스트
  - 통합 테스트 : 서로 구분되는 영역이 함께 잘 작동하는지 확인하는 테스트
...
#### 7.4 Headless UI
  - 컴포넌트를 만들려면 자바스크립트 코드를 작성해야함
  - 이러한 문제를 위해 Headless UI탄생
  - 동적 컴포넌트만 제공

#### 7.3 TailwindCSS
  - CSS규칙만 제공
  - 모듈, 컴포넌트 제공이 안되기 때문에 직접 만들어야 함
  - 개성있는 디자인 가능
...
#### Chapter 7 UI라이브러리
  - 프레임워크, 유틸리티 기능이 필수는 아님
  - 생산성 향상 및 일괄성 유지 탁월

#### 6.3 SASS
  - 기본으로 지원하는 전처리기
  - 패키지 설치 [ $ npm install sass ]
  - SASS 및 SCSS 문법으로 CSS Moddule을 만들고 사용 가능

#### 6.2 CSS Module
  - CSS-in-JS의 단점을 회피하기 위한 좋은 방법

#### CSS-in-JS의 단점
  - 개발 도구 지원 부족
  - 하이라이팅, 자동 완성, 린트 기능 제공X
  - 앱 번들이 커지고 느려짐
  - 하이드레이션이 끝나면 CSS를 다시 생성

#### 6.1 Styled JSX
  - Styled JSXSMS CSS-in-JS 라이브러리
  - 내장 모듈
  - CSS속성 지정을 위해 자바스크립트를 사용할 수 있는 라이브러리

#### Chapter 6 CSS와 내장 스타일링 메서드
  - 모듈화, 유지보수 쉬움, 성능이 뛰어난  모듈

## 1130(14주차)

## 작성코드
  - boilerplate프로젝트 Redux소스 코드 수정
## 학습내용
